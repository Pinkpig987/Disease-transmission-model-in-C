...
void spreadDisease(Graph* graph, int* initialInfectedNodes, int initialInfectedCount, float transmissionRate, float closedTransmissionRate,
	float recoveryRate, float reinfectionRate, float mortalityRate, float isolationRate, float vaccineEfficacy, float vaccinationRate) {
	...
	while (infectedCount > 0) {
		...
		printf("Day %d: SUSCEPTIBLE=%d, INFECTED=%d, ISOLATED=%d, RECOVERED=%d, DEAD=%d, VACCINATED=%d\n", days, susceptibleCount, infectedCount, isolatedCount, recoveredCount, deadCount, vaccinatedCount);
...
int main() {
	...
	spreadDisease(graph, initialInfectedNodes, initialInfectedCount, transmissionRate, closedTransmissionRate, recoveryRate, reinfectionRate, mortalityRate, isolationRate, vaccineEfficacy, vaccinationRate); /* 模擬疾病傳播 */
